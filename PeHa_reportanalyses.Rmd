...
```{r}
fig2d_vpdmax <- ggplot(peha_all[peha_all$Year>1994,], aes(Year, vpdmax, colour=Site)) +
  geom_point(position=position_dodge(0.2), shape=21, fill="white") +
  stat_summary(fun.y = function(x) mean(x, na.rm = TRUE), geom="line") +
  theme_bw() +
...
fig2d_tmin <- ggplot(peha_all[peha_all$Year>1994,], aes(Year, Tmin, colour=Site)) +
  geom_point(position=position_dodge(0.2), shape=21, fill="white") +
  stat_summary(fun.y = function(x) mean(x, na.rm = TRUE), geom="line") +
  theme_bw() +
...
```{r}
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig2.jpg", sep=""), width=300, height=400, units="mm", res=300)


multiplot(Fig2a, Fig2b, Fig2c, Fig2d, fig2d_vpdmax,fig2d_tmin,fig2d_tmax, 
...
Fig4b <- ggplot(peha_all, aes(Tmax, Fl, colour = Site)) +
  geom_point() +
  geom_smooth(method ="lm") +
  scale_color_manual(values = c("black","red")) +
  theme_bw() +
...
```{r}
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig4.jpg", sep=""), width=250, height=370, units="mm", res=300)


multiplot(Fig4a, Fig4a_2, Fig4b, Fig4c,cols=1)
...
Get number of individuals with any fruit rather than the number of fruit that the output was swtiched to in 2012 (after 2012)
```{r}
...
lm1 <- lm(Fl ~ Site*Tmin, peha_all)
lm2 <- lm(Fl ~ Site*Tmax, peha_all)
lm3 <- lm(Fl ~ Site*vpdmax, peha_all) # how much vapor deficit
lm4 <- lm(Fl ~ Site*x, peha_all) # x is precipitation
lm5 <- lm(Fl ~ Tmin, peha_all)
lm6 <- lm(Fl ~ Tmax, peha_all)
lm7 <- lm(Fl ~ x, peha_all)
lm8 <- lm(Fl ~ vpdmax, peha_all)
lm9 <- lm(Fl ~ 1, peha_all)
...
lm1 <- lm(Br ~ Site*Tmin, peha_all)
lm2 <- lm(Br ~ Site*Tmax, peha_all)
lm3 <- lm(Br ~ Site*vpdmax, peha_all) # how much vapor deficit
lm4 <- lm(Br ~ Site*x, peha_all) # x is precipitation
lm5 <- lm(Br ~ Tmin, peha_all)
lm6 <- lm(Br ~ Tmax, peha_all)
lm7 <- lm(Br ~ x, peha_all)
lm8 <- lm(Br ~ vpdmax, peha_all)
lm9 <- lm(Br ~ 1, peha_all)
