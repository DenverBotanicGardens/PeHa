...
#Should be some apply function changing these all to numeric
lapply(peha_m[,1:6], function(x) as.numeric(x))
do.call()

peha_m$TotRos <- as.numeric(peha_m$TotRos)
peha_m$Fl <- as.numeric(peha_m$Fl)
peha_m$Br <- as.numeric(peha_m$Br)
...
#####Now that I added zeros for the two DL transects missing data (needing zeros), these functions aren't working!!  Oh! Turned them into characters instead of numeric!
```{r}
...
for(yr in 1:(length(yrs)-1)){
  t.listae[[yr]] <- t.test(Br~Year, 
                        data = subset(ae, Year == yrs[yr] | Year == yrs[yr+1]))
}
