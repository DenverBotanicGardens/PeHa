...
Fruit information not in summary or raw output for 2017 or 2019
Lost Fruit information. Need it both as total fruit counted and a yes or no if fruit produced
```{r}
'%ni%' <- Negate('%in%')
library(dplyr)

peha[peha$Year == 2019,] # is all missing Fruit

# 2017
peha2017fruit <- read.csv("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Raw Data/2017/20170621_DataForUpload.csv", skip=2)


peha2017fruit <- peha2017fruit[peha2017fruit$X != "",]
peha2017fruit <- peha2017fruit[peha2017fruit$Transect != "Transect",]
peha2017fruit <- peha2017fruit %>% filter(Site != "Site") %>% droplevels()

peha2017fruit[,c(2:9)] <- lapply(peha2017fruit[,c(2:9)], function(x) as.numeric(as.character(x)))
peha2017fruit[,c(1,10)] <- lapply(peha2017fruit[,c(1,10)], function(x) as.character(x))
str(peha2017fruit)
nrow(peha[peha$Year == 2017,]) # 503
nrow(peha2017fruit) # 503

# 2019
peha2019fruit <- read.csv("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Raw Data/2019/EnteredRawData_MaeveWilder_2019.csv", skip=2)

peha2019fruit <- peha2019fruit[peha2019fruit$X != "",]
# peha2019fruit <- peha2019fruit[peha2019fruit$Transect != "Transect",]
peha2019fruit <- peha2019fruit %>% filter(Site != "Site") %>% droplevels()
peha2019fruit <- peha2019fruit %>% filter(Transect != "Transect") %>% droplevels()

peha2019fruit[,c(2:9)] <- lapply(peha2019fruit[,c(2:9)], function(x) as.numeric(as.character(x)))
peha2019fruit[,c(1,10)] <- lapply(peha2019fruit[,c(1,10)], function(x) as.character(x))
str(peha2017fruit)

nrow(peha[peha$Year == 2019,]) # 696
nrow(peha2019fruit) # 696

table(peha2019fruit$Transect[peha2019fruit$Site == "Above Eagle"])

peha2017fruit$Year <- 2017
peha2019fruit$Year <- 2019
pehafruit <- rbind(peha2017fruit, peha2019fruit)
nrow(pehafruit) # 1199
table(pehafruit$Site, pehafruit$Transect, pehafruit$Year)


# pehafruit matchs but after the merge Above Eagle transect 17 is lost and results assigned elsewhere
peha.foo <- merge(peha, pehafruit[,c("Site","Year","Transect","X","Y","X..of.Rosettes",
                   "flowered..br....0.or.1.","X.Fr.Fl")], # removes transect id and browsed
                  by.x = c("Site","Year","Transect","X","Y","X.ofRosettes","Fl"),
                  by.y = c("Site","Year","Transect","X","Y","X..of.Rosettes","flowered..br....0.or.1."),
                  all.x = TRUE)
# peha.foo <- merge(peha, pehafruit[,c("Site","Year","Transect","X","Y","X..of.Rosettes",
#                                      "flowered..br....0.or.1.","X.Fr.Fl")], 
#                   by = c("Site","Year","Transect","X","Y"),
#                   all.x = TRUE)
peha.foo$X.Fr.Fl <- as.numeric(as.character(peha.foo$X.Fr.Fl))

table(peha.foo$Site)
table(peha$Site) # the additional are in Above Eagle

table(peha.foo$Year)
table(peha$Year) # the additional are in Above Eagle, 2019

table(pehafruit$Site, pehafruit$Transect, pehafruit$Year)

table(peha.foo$Transect[peha$Site == "Above Eagle" & peha$Year == 2019])
table(peha$Transect[peha$Site == "Above Eagle" & peha$Year == 2019]) # the additional are in Above Eagle, 2019, peha.foo lost transect 17 and assigned a bunch to other transects! 

nrow(peha.foo) # 13615
nrow(peha)     # 13613

peha.foo$Fr_Fl <- rowSums(peha.foo[,c("Fr_Fl","X.Fr.Fl")], na.rm=TRUE)

peha.foo[peha.foo$Year > 2016,]
peha.foo <- peha.foo[,-length(peha.foo)]

# Just this time before it gets fixed
identical(peha.foo %>% arrange(Site, Year, Transect, X, Y, X.ofRosettes) , 
peha %>% arrange(Site, Year, Transect, X, Y, X.ofRosettes) )

peha[peha$Year==2019 & peha$Transect == 17,] %>% arrange(Site, Year, Transect, Y) %>% select(1:9)
peha.foo[peha.foo$Year==2019 & peha.foo$Transect == 17,] %>% arrange(Site, Year, Transect, Y) %>% select(1:9)


setdiff(peha.foo, peha) # 1201 are different; there are 1199 2017 and 2019 rows; still 2 additional rows
intersect(peha.foo[,1:7],peha[,1:7]) # 13597, missing 16 rows
setdiff(peha.foo[,1:7],peha[,1:7]) # All are the same

# Not yet! They are different, garrrr
# peha <- peha.foo
...
# x is sum of precip over all preceeding months
...
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig2a_rossets.jpg", sep=""), width=175, height=125, units="mm", res=300)
...
```{r}
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig2b_percentrep.jpg", sep=""), width=175, height=125, units="mm", res=300)
...
```{r}
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig2c_herbivory.jpg", sep=""), width=175, height=125, units="mm", res=300)
...
```{r}
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig2d_precip.jpg", sep=""), width=175, height=125, units="mm", res=300)
...
```{r}
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig2.jpg", sep=""), width=400, height=275, units="mm", res=300)


multiplot(Fig2a, Fig2b, Fig2c, Fig2d, cols=2)
...
```{r}
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig3a_count.jpg", sep=""), width=175, height=125, units="mm", res=300)
...
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig3b_count.jpg", sep=""), width=175, height=125, units="mm", res=300)
...
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig4a_count.jpg", sep=""), width=175, height=125, units="mm", res=300)
...
```{r}
Fig4b <- ggplot(peha_all, aes(x, Fl, colour = Site)) +
  geom_point() +
  geom_smooth(method ="lm") +
  scale_color_manual(values = c("black","red")) +
  theme_bw() +
...
```{r}
Fig4c <- ggplot(peha_all, aes(x, Br, colour = Site)) +
  geom_point() +
  geom_smooth(method ="lm") +
  scale_color_manual(values = c("black","red")) +
  theme_bw() +
...
```{r}
jpeg(paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/Reports/Word ",currentyr, "_PeHa/Fig4.jpg", sep=""), width=250, height=400, units="mm", res=300)
...
write.csv(Appendix, file=paste("Q:/Research/All_Projects_by_Species/Penstemon SPECIES/Penstemon_harringtonii/R_code_PeHa/R_tables/",currentyr,"Appendix.csv", sep=""))
...
# Average rosettes per year over study
mean(dl.app$total)
mean(ae.app$total)
