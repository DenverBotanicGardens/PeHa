...
Using these count estimates, the population growth rate (λ) at Above Eagle is 0.99 (95% CI: 0.94-1.04) and at Dry Lake is 0.99 (95% CI: 0.84-1.18).     
```{r}
...
#rows are subjects and columns are levels of repeated measures factor - site and year
site.year <- apply(expand.grid(unique(peha_all_sum$Site),unique(peha_all_sum$Year)),1,
      function(x) paste(x[1],x[2],sep="."))

#peha.aov.ros2 <- Anova(peha_all_sum, idata = )

#library(ez)


#ezANOVA(data = peha_all_sum, wid=.(Site), dv = .(TotRos),
#        within = .(Year,Site), between = .(Year), observed = .(Year))

peha.aov.fl <-aov(asin(sqrt(PerRepro/100))~Year*Site + Error(Plot), data=peha) 
summary(PeHa.aov) 
```
