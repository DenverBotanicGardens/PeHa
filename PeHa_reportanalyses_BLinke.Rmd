...
DoubleObs <- read.csv("C:/Users/deprengm/Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Penstemon-harringtonii/Penstemon-harringtonii_RawData/2025/2025_Penstemon-harringtonii_RawDataS123/Penstemon_harringtonii_DemographicMonitoring_0.csv")
DoubleObs <- DoubleObs %>%
  dplyr::group_by(Site, Transect) %>%
  summarise(NumMissedByPrimary = sum(Number.Plants.Missed.By.Primary.Observer),
            PrimaryObs = paste(Primary.Observer.s.Initials, collapse = ","),
            SecondObs = paste(Secondary.Observer, collapse = ",")) 

plants <- read.csv("C:/Users/deprengm/Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Penstemon-harringtonii/Penstemon-harringtonii_RawData/2025/2025_Penstemon-harringtonii_RawDataS123/plants_1.csv")

peha2025 <- DoubleObs %>%
  left_join(plants, by = c("Transect" = "MySQL.Transect.ID")) %>%
  dplyr::select(Site.y, Transect, Transect.y, X:X.Fr.Fl,NumMissedByPrimary, PrimaryObs, SecondObs)
...
peha.c %>%
  filter(Year == 2015) %>%
  group_by(Site) %>%
  summarise(vpdmax = min(vpdmax))

peha.c %>%
  filter(Year == 2015) %>%
  group_by(Site) %>%
  summarise(vpdmaxmean = mean(vpdmax))
...
peha_sum <- peha %>%
  ...
  summarise(TotRos = sum(X.ofRosettes),
            Fl = sum(Fl)/n(),
            Br = sum(Br)/n())
...
```{r}
# peha_sumSE <- summarySE(peha_sum, measurevar="TotRos", groupvars=c("Site","Year"))
