...
```{r}
rm(list=ls())
setwd("C:/Users/deprengm/Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Penstemon-harringtonii")
...
library(nlme)
library(car)
# library(Rmisc) # Because it requires plyr and that breaks the dplyr group_by
library(patchwork)
library(dplyr)
library(tidyr)
...
## Survey123 to upload format   
```{r}
## Transect is the MySQL Transect ID, repeats are when two teams completed one plot
DoubleObs <- read.csv(paste("Penstemon-harringtonii_RawData/",currentYr,"/",currentYr,"_Penstemon-harringtonii_RawDataS123/Penstemon_harringtonii_DemographicMonitoring_0.csv", sep = ""))
DoubleObs <- DoubleObs %>%
  dplyr::group_by(Site, Transect) %>%
  dplyr::summarise(NumMissedByPrimary = sum(Number.Plants.Missed.By.Primary.Observer),
            PrimaryObs = paste(Primary.Observer.s.Initials, collapse = ","),
            SecondObs = paste(Secondary.Observer, collapse = ",")) 

plants <- read.csv(paste("Penstemon-harringtonii_RawData/",currentYr,"/",currentYr,"_Penstemon-harringtonii_RawDataS123/plants_1.csv",
                         sep=""))

peha2025 <- DoubleObs %>%
  left_join(plants, by = c("Transect" = "MySQL.Transect.ID")) %>%
  dplyr::select(Site.x, Transect, Transect.y, X:X.Fr.Fl,NumMissedByPrimary, PrimaryObs, SecondObs)

### Need to remove the ones where primary and secondary match (me, I did some myself so no missed)
peha2025 %>%
  filter(PrimaryObs != SecondObs) %>%
  group_by(Transect, NumMissedByPrimary) %>%
  dplyr::summarise(n = n())


## ?????
toUpload <- peha2025 %>%
  dplyr::select(Site.x, MySQL.Transect.ID, Transect, X..of.Rosettes, 
         X,Y, X..of.Rosettes, flowered..br....0.or.1.,
         X.Fr.Fl, Browsed..br....0.or.1., Comments)


plants %>%
  dplyr::select(Site, MySQL.Transect.ID, Transect, X,Y, 
                X..of.Rosettes, flowered..br....0.or.1., 
                X.Fr.Fl, Browsed..br....0.or.1., Comments) %>%
  # group_split(Site, Transect, .keep =TRUE) %>%
  arrange(Site, Transect, Y) %>%
  write.csv("C:/Users/deprengm/Denver Botanic Gardens/Conservation - General/AllProjectsBySpecies/Penstemon-harringtonii/Penstemon-harringtonii_RawData/2025/2025_Penstemon-harringtonii_RawDataS123/toUpload.csv")
...
https://prism.nacse.org/documents/PRISM_datasets.pdf 
```{r}
...
names(peha.c) <- c("Date","Precip","Tmin","tmean","Tmax","tdewmean","vpdmin","vpdmax","Site","Year","Month")
...
peha.annual <- peha.c %>%
  ...
  ## When Month > 7, then that goes to the next year
  mutate(Prev12 = case_when(Month >= 7 ~ (Year+1),
                            TRUE ~ Year)) %>%
  group_by(Prev12, Site) %>%
  dplyr::summarise(Precip = sum(Precip),
            tmin = min(Tmin),
            tminavg = mean(Tmin),
            tmax = max(Tmax),
            tmaxavg = mean(Tmax),
            DewPtmean = mean(tdewmean),
            vpdminmean = mean(vpdmin),
            vpdmin = min(vpdmin),
            vpdmaxmean = mean(vpdmax),
            vpdmax = max(vpdmax))
# peha.annual <- aggregate(.~Year + Site, peha.c[,c(2:9)], function(x) mean(x, na.rm=TRUE))
tail(peha.annual)

peha.c %>%
  filter(Year == 2015) %>%
  group_by(Site) %>%
  dplyr::summarise(vpdmax = min(vpdmax))

peha.c %>%
  filter(Year == 2015) %>%
  group_by(Site) %>%
  dplyr::summarise(vpdmaxmean = mean(vpdmax))

peha.annual %>%
ggplot( aes(Prev12, Precip, colour = Site))+
...
peha.annual %>%
  # filter(Prev12 > 1993) %>%
ggplot(   aes(Prev12, vpdmax, colour = Site))+
  stat_summary(fun = mean, geom="line")+
  theme_bw()+
...
peha.annual %>%
  # filter(Prev12 > 1993) %>%
ggplot(   aes(Prev12, vpdmaxmean, colour = Site))+
  stat_summary(fun = mean, geom="line")+
  theme_bw()+
  stat_smooth(method = "glm", se=TRUE)
...
peha.annual %>%
  # filter(Prev12 > 1993) %>%
ggplot(   aes(Prev12, vpdmin, colour = Site))+
  stat_summary(fun = mean, geom="line")+
  theme_bw()+
  stat_smooth(method = "glm", se=TRUE)
...
## Number of rosettes counted in current year
sum(peha_summary$Sumofrosettes[peha_summary$Year == currentyr])

peha_summary <- peha_summary %>%
  dplyr::select(Year:Sumofbrowsed)
...
peha %>%
  group_by(Site,Year) %>%
  dplyr::summarise(Ros = sum(X.ofRosettes)) %>%
  print(n = 50)
...
peha %>%
  ...
  dplyr::summarise(Ros = sum(X.ofRosettes)) %>%
  group_by(Site) %>%
  dplyr::summarise(Totmean = mean(Ros))


peha_sum <- peha %>%
  group_by(Site, Year, Transect) %>%
  dplyr::summarise(TotRos = sum(X.ofRosettes),
            Fl = sum(Fl)/n(),
            Br = sum(Br)/n())
...
peha_all <- peha %>%
  ...
  dplyr::summarise(TotRos = sum(X.ofRosettes),
                   ...
                   NumFlFr = sum(Fr_Fl),
                   PerBr = (sum(Br)/n())) %>%
  left_join(peha.annual, by = c("Year" = "Prev12", "Site" = "Site"))
...
summarySE(peha.annual, measurevar = "Precip", groupvars = "Site")
...
700 x 450
```{r}
## Make sure zeros included when needed, need to fill with zero

Fig2a <- peha_sum %>%
  group_by(Site, Year) %>%
  dplyr::summarise(meanRos = mean(TotRos),
            sdRos = sd(TotRos),
            SERos = sqrt(var(TotRos)/n()),
            CIlwrRos = meanRos - (1.96 * SERos),
            CIuprRos = meanRos + (1.96 * SERos)) %>%
  ggplot( aes(Year, meanRos, colour=Site)) +
  geom_point(position=position_dodge(0.1), shape=21, fill="white") +
  geom_line(position=position_dodge(0.1)) +
  geom_errorbar(aes(ymin=CIlwrRos, ymax= CIuprRos),width=1, position = position_dodge(0.1)) +
  theme_bw() +
...
Fig2b <- peha_sum %>%
  group_by(Site, Year) %>%
  dplyr::summarise(mean = mean(Fl),
            sd = sd(Fl),
            SE = sqrt(var(Fl)/n()),
            CIlwr = mean - (1.96 * SE),
            CIupr = mean + (1.96 * SE)) %>%
  ggplot( aes(Year, mean, colour=Site)) +
  geom_point(position=position_dodge(0.1), shape=21, fill="white") +
  geom_line(position=position_dodge(0.1)) +
  geom_errorbar(aes(ymin=CIlwr, ymax= CIupr),width=1, position = position_dodge(0.1)) +
  theme_bw() +
...
ggsave(paste(userpath,currentyr,"_Penstemon-harringtonii_Fig2.jpg", sep=""),
     ...
     units = "mm", dpi = 300, height = 350, width = 300)   
...
peha.seasonal <- peha.c %>%
  mutate(Year = as.numeric(Year)) %>%
  mutate(Prev12 = case_when(Month >=7 ~ (Year + 1),
                            TRUE ~ Year)) %>%
  mutate(season = case_when((Month > 9 & Month < 13) ~ "winter",
                            (Month > 0 & Month < 4) ~ "spring",
...
peha.c.summary <- peha.seasonal %>%
  group_by(Site, Prev12, season) %>%
  dplyr::summarise(totppt = sum(Precip),
                   tmin.mean = mean(Tmin),
                   tmax.mean = mean(Tmax),
                   vpdmax.mean = mean(vpdmax),
                   vpdmin.mean = mean(vpdmin),)

# This has all same info except averaged over years (previous 12 months) instead of seasons and has demographic data
peha_all_seasonal <- peha_all %>%
  left_join(peha.c.summary, by = c("Year" = "Prev12", "Site" = "Site"))
...
Figure 2. Annual growth rate estimates with 95% confidence intervals for consectutive additional years of demographic study    
```{r}
...
peha_FlYN <- peha_FlYN %>%
  ungroup() %>%
  mutate(scaleppt = scale(x),
         scaletmin = scale(Tmin),
         scaletmax = scale(Tmax),
         scalevpdmax = scale(vpdmax))
...
peha_FlYN <- peha_FlYN %>%
  dplyr::mutate(Br = if_else(Br >= 1, 1,0))

# Fig4a <- ggplot(peha_all, aes(x, TotRos, colour = Site)) +
#   geom_point() +
...
# Rosettes is precipitation   
## Year == Prev12 months prior to July
Fig4a <-
  peha %>%
  group_by(Site, Year, Transect) %>%
  dplyr::summarise(TotRos = sum(X.ofRosettes),
                   ...
                   NumFlFr = sum(Fr_Fl),
                   PerBr = (sum(Br)/n())) %>%
  left_join(peha.annual, c("Site" = "Site", "Year" = "Prev12")) %>%
  
ggplot(    aes(Precip, TotRos, colour = Site)) +
  geom_point() +
  geom_smooth(method ="glm") +
  scale_color_manual(values = c("black","red")) +
  theme_bw() +
  xlab("") +
  ylab(expression(paste("Average rosettes per 60m"^2))) +
  # xlab("Maximum Temperature (degree C)")+
...
ggplot(    aes(tmax, Fl, color = Site))+
  ...
  # xlab("Maximum Temperature (degree C)")+
  xlab("Annual precipitation (mm)") +
  theme(legend.justification=c(1,0.90), legend.position=c(1,0.90),
        legend.background = element_rect(fill=alpha('white', 0.1)))+
  ggtitle("b)") +
  theme(plot.title=element_text(hjust=0))
...
Fig4c <- 
  peha %>%
  left_join(peha.annual, c("Site" = "Site", "Year" = "Prev12")) %>%
  mutate(Br = case_when(Br > 0 ~ 1,
                        TRUE ~ 0)) %>%
ggplot(   aes(vpdmax, Br, color = Site))+ 
  geom_point(pch = "|")+
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial"), 
              se = TRUE)+
...
Gaussian state-space model via Schaub & Kery 2022   
```{r}
library(jagsUI)
library(dplyr)
library(tidyr)
...
peha_GSSM <- peha %>%
  group_by(Site, Year) %>%
  dplyr::summarise(Plants = n(),
            Rosettes = sum(X.ofRosettes),
            Vegetative = sum(X.ofRosettes[Fl == 0]),
            Reproductive = sum(X.ofRosettes[Fl == 1]),
            Fruit = sum(Fr_Fl))

## ABOVE EAGLE
# y <-  peha_GSSM$Rosettes[peha_GSSM$Site == "Above Eagle"]
nyears <- length(peha_GSSM$Site[peha_GSSM$Site == "Above Eagle"])

jags.data <- list(y = peha_GSSM$Rosettes,
                  nsites = 2, nyears = nyears)  
...
cat(file = "model1.txt", "
    ...
    # Process model - population dynamics model
    for (t in 1:(T-1)){
...
plot(y, col = "blue", pch = 20)
# lines(y)
arrows(1:length(y), y, 1:length(y), out1$mean$N, length = 0.05)
points(out1$mean$N)
lines(out1$mean$N, col = "grey")
lines(out1$q97.5$N, col = "red")
lines(out1$q2.5$N, col = "red")
...
## Dry Lake
y <-  peha_GSSM$Rosettes[peha_GSSM$Site == "Dry Lake"]
...
## Did we start taking data on the number of fruit and flowers instead of the ones that did at all? 
peha %>%
  group_by(Site, Year) %>%
  dplyr::summarise(NumFr = sum(Fr_Fl > 1)) %>%
  print(n = 50)

peha <- peha %>%
  mutate(Fr_FlNum = if_else(Fr_Fl == 0, 0, 1))
...
Appendix.list <- lapply(split(peha, list(peha$Site,peha$Year)),
                   function(x) do.call(rbind,lapply(split(x, x$Transect),
                                      function(y) data.frame(Plot=unique(y$Transect),
                                                             ...
                                                             Flowered=sum(y$Fl),
                                                             Fruited=sum(y$Fr_FlNum, na.rm=TRUE),
                                                             Herbivory=sum(y$Br)))))
...
peha_all <- peha_all %>%
  mutate(scaletmin = c(scale(tmin)),
         scaletmax = c(scale(tmax)),
         scaleppt = c(scale(Precip)),
         scalevpdmax = c(scale(vpdmax)))

lm1 <- glm(TotRos ~ Site*scaletmin, family = poisson, peha_all)
lm2 <- glm(TotRos ~ Site*scaletmax, family = poisson,peha_all)
lm3 <- glm(TotRos ~ Site*scalevpdmax, family = poisson,peha_all) # how much vapor deficit
lm4 <- glm(TotRos ~ Site*scaleppt, family = poisson,peha_all) # 
lm5 <- glm(TotRos ~ scaletmin, family = poisson,peha_all)
lm6 <- glm(TotRos ~ scaletmax, family = poisson,peha_all)
lm7 <- glm(TotRos ~ scaleppt,family = poisson, peha_all)
lm8 <- glm(TotRos ~ scalevpdmax,family = poisson, peha_all)
...
Use binomial but merge data to peha (not summarized by plot peha_all)
...
lm1 <- glm(Fl ~ Site*scaletmin, family = binomial(link = "logit"), peha_FlYN)  #   #2
lm2 <- glm(Fl ~ Site*scaletmax, family = binomial(link = "logit"),peha_FlYN) #     #3
lm3 <- glm(Fl ~ Site*scalevpdmax, family = binomial(link = "logit"),peha_FlYN) # how much vapor deficit
lm4 <- glm(Fl ~ Site*scaleppt, family = binomial(link = "logit"),peha_FlYN) # x is precipitation
lm5 <- glm(Fl ~ scaletmin, family = binomial(link = "logit"),peha_FlYN)
lm6 <- glm(Fl ~ scaletmax, family = binomial(link = "logit"),peha_FlYN) 
lm7 <- glm(Fl ~ scaleppt, family = binomial(link = "logit"),peha_FlYN) #             #1
lm8 <- glm(Fl ~ scalevpdmax, family = binomial(link = "logit"),peha_FlYN)
lm9 <- glm(Fl ~ 1, family = binomial(link = "logit"),peha_FlYN)
...
lm1 <- glm(Br ~ Site*scaletmin, family = binomial(link = "logit"),peha_FlYN)
lm2 <- glm(Br ~ Site*scaletmax,family = binomial(link = "logit"), peha_FlYN)
lm3 <- glm(Br ~ Site*scalevpdmax,family = binomial(link = "logit"), peha_FlYN) # how much vapor deficit
lm4 <- glm(Br ~ Site*scaleppt, family = binomial(link = "logit"),peha_FlYN) # x is precipitation
lm5 <- glm(Br ~ scaletmin,family = binomial(link = "logit"), peha_FlYN)
lm6 <- glm(Br ~ scaletmax,family = binomial(link = "logit"), peha_FlYN)
lm7 <- glm(Br ~ scaleppt,family = binomial(link = "logit"), peha_FlYN)
lm8 <- glm(Br ~ scalevpdmax, family = binomial(link = "logit"),peha_FlYN)
lm9 <- glm(Br ~ 1,family = binomial(link = "logit"), peha_FlYN)
...
summary(lm3)
sum(lm3$coefficients[c(3,4)])
```


# Percent of reproductive individuals   
There is a significantly greater proportion of reproductive individuals found at Above Eagle than at Dry Lake 
```{r}
